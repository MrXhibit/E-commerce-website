...
const ProfilePage = () => {
  ...
  if (!user) {
    return (
      <>
        <Header />
        <Box sx={{ background: '#f4f6fa', minHeight: '100vh', py: 6, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Card sx={{ p: 4, textAlign: 'center' }}>
            <Typography variant="h5" gutterBottom>Please log in to view your account</Typography>
  ...
  const renderEditDialog = () => {
    ...
    return (
      <Dialog open={editDialogOpen} onClose={() => setEditDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>Edit {editSection === 'account' ? 'Account Details' : editSection === 'address' ? 'Shipping Address' : 'Payment Method'}</DialogTitle>
        <DialogContent>
          {getDialogContent()}
        </DialogContent>
        <DialogActions>
            ...
            <Grid xs={12} md={9}>
                ...
                <Grid xs={12} md={4}>
                  <Card sx={{ p: 3, boxShadow: 2, position: 'relative' }}>
                    <IconButton 
                      ...
                      onClick={() => handleEditClick('account', { userName: user.userName, email: user.email })}
                    >
                ...
                <Grid xs={12} md={4}>
                  <Card sx={{ p: 3, boxShadow: 2, position: 'relative' }}>
                    <IconButton 
                      size="small" 
                      sx={{ position: 'absolute', top: 12, right: 12 }}
                      onClick={() => handleEditClick('address', { address: '', city: '', country: '', state: '', zip: '' })}
                    >
                      <EditIcon fontSize="small" />
                    </IconButton>
                    <Typography variant="subtitle1" fontWeight={700} sx={{ mb: 2 }}>Shipping Address</Typography>
                    <Typography variant="body2" color="text.secondary">Address</Typography>
                    <Typography variant="body1" fontWeight={600}>Not set</Typography>
                ...
                <Grid xs={12} md={4}>
                  <Card sx={{ p: 3, boxShadow: 2, position: 'relative' }}>
                    <IconButton 
                      size="small" 
                      sx={{ position: 'absolute', top: 12, right: 12 }}
                      onClick={() => handleEditClick('payment', { cardHolder: '', cardNumber: '', cardExpiry: '' })}
                    >
                      <EditIcon fontSize="small" />
                    </IconButton>
                    <Typography variant="subtitle1" fontWeight={700} sx={{ mb: 2 }}>Payment Methods</Typography>
                    <Typography variant="body2" color="text.secondary">No payment methods added</Typography>
                    <Button variant="outlined" size="small" sx={{ mt: 2 }} fullWidth>
                      Add Payment Method
                    </Button>
                  </Card>
              ...
              <Card sx={{ mt: 4, p: 3, boxShadow: 2 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                  <Typography variant="h6" fontWeight={700}>Wish List ({wishlist?.itemCount || 0} items)</Typography>
