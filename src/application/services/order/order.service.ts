...
export class OrderService implements OrderServiceInterface{
  ...
  // Add missing methods for payment controller
  async updatePaymentIntentId(orderId: string, paymentIntentId: string): Promise<orderProperties> {
    const order = await this.orederRepo.findById(orderId);
    if (!order) {
  ...
  async updatePaymentStatus(orderId: string, status: string, transactionId?: string): Promise<orderProperties> {
    const order = await this.orederRepo.findById(orderId);
    if (!order) {
      throw new Error('Order not found');
    }
